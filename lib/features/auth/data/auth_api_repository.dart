...
class AuthApiRepository {
  ...
  String get tapovanaBaseUrl => ApiConfig.authProfileBackendUrl;
  ...
  // Forgot password routes live on the Tapovana Render backend (tapovanaBaseUrl),
  // NOT on the Rosette backend, which would reject them with "Missing token".

  Future<Map<String, dynamic>> sendForgotPasswordOtp({required String email}) async {
    final response = await http.post(
      Uri.parse('$tapovanaBaseUrl/api/auth/forgot-password/send-otp'),
      headers: {"Content-Type": "application/json"},
  ...
  Future<Map<String, dynamic>> verifyForgotPasswordOtp({required String email, required String otp}) async {
    final response = await http.post(
      Uri.parse('$tapovanaBaseUrl/api/auth/forgot-password/verify-otp'),
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({"email": email, "otp": otp}),
    );
  ...
  Future<Map<String, dynamic>> resetPassword({required String email, required String newPassword}) async {
    final response = await http.post(
      Uri.parse('$tapovanaBaseUrl/api/auth/forgot-password/reset'),
      headers: {"Content-Type": "application/json"},
